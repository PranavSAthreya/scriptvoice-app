...
//   const [activeCharacter, setActiveCharacter] = useState('LISA');
...
//   // Cast Profiles for Voice Matching (Mode 2)
...
//   // Modal States for Add Actor & Clear Script (Replaces blocked browser prompt/confirm)
//   const [showAddActorModal, setShowAddActorModal] = useState(false);
//   const [newActorNameInput, setNewActorNameInput] = useState('');
...
//         e.preventDefault();
//         handleAiPolishScript();
//       }
//     };

//     window.addEventListener('keydown', handleKeyDown);
//     return () => window.removeEventListener('keydown', handleKeyDown);
//   }, [isRecording, activeCharacter, castProfiles, scriptItems]);
...
//       stopAudioProcessing();
//     };
//   }, [mode, castProfiles, activeCharacter, keywords, isRecording]);
...
//             setActiveCharacter(closestChar.name);
//           }
//         } else {
//           setLivePitch(0);
//         }
...
//     } else if (mode === 'cast') {
//       const speakerName = matchedCastName || activeCharacter || 'ACTOR';
...
//     showToast('Manuscript cleared.', 'info');
//   };
...
//       showToast(`Imported ${parsedElements.length} lines from ${filename || 'script'}`, 'success');
//     } else {
...
//     const demo = `INT. RECORDING STUDIO - NIGHT

// Lisa sits across from John at a polished mahogany table.

// LISA
// Is this dictaphone capturing our full manuscript?

// JOHN
// (whispering into ribbon mic)
// Every single line. Speak naturally.`;
//     parseAndLoadScript(demo, 'Demo Manuscript');
//   };
...
//     setNewActorNameInput('');
//     setShowAddActorModal(true);
//   };

//   const confirmAddCastMember = () => {
//     const formattedName = newActorNameInput.trim().toUpperCase();
...
//       color: 'bg-amber-500'
//     }]);
//     setActiveCharacter(formattedName);
//     setShowAddActorModal(false);
//     setNewActorNameInput('');
//     showToast(`Added channel ${channelNum}: ${formattedName}`, 'success');
//   };

//   // --- AI Script Doctor (Gemini 2.5 Flash API) ---
//   const handleAiPolishScript = async () => {
//     if (scriptItems.length === 0) {
//       showToast('No dictated lines to format!', 'error');
//       return;
//     }

//     setIsAiProcessing(true);
//     const rawScriptText = scriptItems.map(item => `${item.type.toUpperCase()}: ${item.text}`).join('\n');

//     const prompt = `You are a professional Hollywood script doctor.
// Format and polish this raw dictation into standard screenplay syntax.
// Clean stuttering, fix formatting, add emotion parentheticals, and clean INT/EXT headers.

// Return JSON in schema:
// {
//   "elements": [
//     { "type": "scene" | "action" | "character" | "parenthetical" | "dialogue", "text": "string" }
//   ]
// }
...
//             const formatted = parsed.elements.map((el, index) => ({
//               id: Date.now() + '-' + index,
//               type: el.type.toLowerCase(),
//               text: el.text
//             }));
//             setScriptItems(formatted);
//             showToast('Manuscript Polished by AI Doctor', 'success');
...
//       {/* Classy Toast Alert */}
//       {toastMessage && (
//         <div className={`fixed top-5 right-5 z-50 flex items-center gap-2 px-5 py-3 border border-[#d4af37]/40 shadow-[0_10px_25px_rgba(0,0,0,0.8)] font-sans text-xs tracking-wider uppercase font-semibold backdrop-blur-md ${
...
//               </div>
//               <div>
//                 <h1 className="text-xl sm:text-2xl font-serif font-normal tracking-[0.2em] text-[#f2e6d8] uppercase drop-shadow">
...
//                   {mode === 'cast' ? (matchedCastName || activeCharacter) : activeCharacter}
//                 </div>
//               </div>
//             </div>
...
//                       Desktop Hotkeys:
//                     </div>
//                     <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10px] text-[#9e8d7c]">
//                       <div><kbd className="bg-[#211a14] px-1 text-[#e8ded1]">Space</kbd> / <kbd className="bg-[#211a14] px-1 text-[#e8ded1]">R</kbd></div>
//                       <div>Record / Pause</div>
//                       <div><kbd className="bg-[#211a14] px-1 text-[#e8ded1]">Tab</kbd></div>
//                       <div>Cycle Speaker</div>
//                       <div><kbd className="bg-[#211a14] px-1 text-[#e8ded1]">Ctrl+Enter</kbd></div>
//                       <div>AI Polish</div>
//                     </div>
//                   </div>
//                 </div>
...
//                   className="text-[10px] text-[#e5c158] hover:underline uppercase font-semibold cursor-pointer"
//                 >
//                   + Add Actor
//                 </button>
//               </div>
...
//                 {castProfiles.map((p) => {
//                   const isSelected = activeCharacter === p.name;
//                   return (
//                     <div
//                       key={p.id}
//                       onClick={() => setActiveCharacter(p.name)}
//                       className={`p-2.5 border transition cursor-pointer font-sans flex items-center justify-between ${
//                         isSelected
...
//                         <span className="text-[9px] font-mono text-[#7a6a5a] font-bold">CH-{p.channel}</span>
//                         <div className={`w-2 h-2 rounded-full ${p.color}`} />
//                         <div>
//                           <span className="text-xs font-semibold text-[#e8ded1] uppercase tracking-wider">{p.name}</span>
//                           <span className="text-[9px] text-[#8a7662] block font-mono">{p.sampleHz} Hz Target</span>
//                         </div>
//                       </div>
...
//                           onClick={(e) => {
//                             e.stopPropagation();
//                             calibrateVoiceProfile(p.id);
//                           }}
//                           disabled={isCalibrating}
//                           className="px-2 py-1 bg-[#14100e] hover:bg-[#221b16] text-[#c8b9a6] border border-[#3d3127] text-[9px] uppercase font-semibold flex items-center gap-1 cursor-pointer"
//                         >
//                           <Volume2 className="w-3 h-3 text-[#d4af37]" />
//                           {isCalibrating && calibratingCharId === p.id ? 'Sampling...' : 'Sample'}
//                         </button>
//                       )}
//                     </div>
//                   );
...
//                   className="py-2.5 px-3 bg-[#2a2012] hover:bg-[#382b19] border border-[#d4af37]/40 text-[#f2da91] font-semibold text-xs flex items-center justify-center gap-2 shadow uppercase tracking-wider disabled:opacity-50 cursor-pointer transition"
//                 >
//                   <Wand2 className={`w-3.5 h-3.5 ${isAiProcessing ? 'animate-spin' : ''}`} />
//                   {isAiProcessing ? 'Formatting...' : 'AI Doctor'}
//                 </button>
//               </div>
//             </div>
...
//             <div className="bg-[#14100e] border border-[#3d3127] p-2.5 flex items-center justify-between gap-2 overflow-x-auto font-sans">
//               <span className="text-[10px] font-semibold text-[#c5a059] uppercase tracking-wider whitespace-nowrap flex items-center gap-1">
...
//                     className="text-red-800 hover:text-red-950 transition cursor-pointer"
//                     title="Clear script"
//                   >
//                     Clear
//                   </button>
...
//               <div className="flex-1 p-8 sm:p-12 overflow-y-auto space-y-4 font-mono text-xs sm:text-sm leading-relaxed max-h-[720px] tracking-tight">
...
//                 </button>
//                 <button
//                   onClick={() => addScriptElement('character', activeCharacter)}
//                   className="px-3 py-1 bg-[#dfcca2] hover:bg-[#cca985] text-[#1a1410] border border-[#b89570] transition cursor-pointer"
//                 >
//                   + Character
//                 </button>
...
//               <span>{mode === 'cast' ? (matchedCastName || activeCharacter) : activeCharacter}</span>
//             </div>
//           </div>
...
//       {/* CUSTOM ADD ACTOR MODAL */}
//       {showAddActorModal && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-sans">
//           <div className="bg-[#1c1713] border-2 border-[#c5a059] p-6 max-w-sm w-full shadow-2xl space-y-4">
//             <h3 className="text-sm font-semibold uppercase tracking-wider text-[#d4af37]">Add Cast Member</h3>
//             <p className="text-xs text-[#a89580]">Enter character name for dictation and channel tracking.</p>
//             <input
//               type="text"
//               value={newActorNameInput}
//               onChange={(e) => setNewActorNameInput(e.target.value.toUpperCase())}
//               placeholder="e.g. SAM"
//               autoFocus
//               onKeyDown={(e) => { if (e.key === 'Enter') confirmAddCastMember(); }}
//               className="w-full bg-[#0a0807] border border-[#3d3127] p-2.5 text-xs font-mono font-bold text-[#e5c158] uppercase focus:border-[#d4af37] focus:outline-none"
//             />
//             <div className="flex justify-end gap-2 text-xs font-semibold uppercase">
//               <button
//                 onClick={() => setShowAddActorModal(false)}
//                 className="px-3 py-1.5 bg-[#120f0d] border border-[#3d3127] text-[#8a7662] hover:text-[#e8ded1] cursor-pointer"
//               >
//                 Cancel
//               </button>
//               <button
//                 onClick={confirmAddCastMember}
//                 className="px-3 py-1.5 bg-gradient-to-r from-[#c5a059] to-[#a8823b] text-black font-bold shadow cursor-pointer"
//               >
//                 Add Character
//               </button>
//             </div>
...
//       {showClearModal && (
//         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 font-sans">
//           <div className="bg-[#1c1713] border-2 border-red-500/60 p-6 max-w-sm w-full shadow-2xl space-y-4">
//             <h3 className="text-sm font-semibold uppercase tracking-wider text-red-400">Clear Manuscript?</h3>
//             <p className="text-xs text-[#a89580]">This action will delete all recorded script lines and reset the tape timer.</p>
//             <div className="flex justify-end gap-2 text-xs font-semibold uppercase">
//               <button
...
//               </button>
//               <button
//                 onClick={confirmClearScript}
//                 className="px-3 py-1.5 bg-red-800 hover:bg-red-700 text-white font-bold shadow cursor-pointer"
//               >
//                 Clear Manuscript
//               </button>
//             </div>
...
export default function App() {
  ...
  // Modal States
  const [modalState, setModalState] = useState({ type: null, isOpen: false }); // types: 'add', 'edit', 'clear', 'delete'
  ...
  useEffect(() => {
    ...
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isRecording, activeCharacter, castProfiles, scriptItems, modalState]);
  ...
  const confirmEditActor = () => {
    ...
    // Update active character if we edited the current one
  ...
  const confirmDeleteActor = () => {
    ...
    // If we deleted the active character, fallback to another
    if (activeCharacter === targetActor.name) {
              ...
              <div className="bg-[#ebd8c0] border-b border-[#cca985] px-6 py-2.5 flex items-center justify-between text-xs text-[#4a3a2a] font-sans font-semibold uppercase tracking-wider">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-red-800 hover:text-red-950 transition cursor-pointer flex items-center gap-1"
                    title="Clear script"
                  >
